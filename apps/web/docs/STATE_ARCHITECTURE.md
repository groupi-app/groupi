...
export async function getCachedData(
  id: string
): Promise<ResultTuple<Error, Data>> {
  try {
...
onInsert: newPost => {
  queryClient.setQueryData(queryKey, old => {
    ...
    const optimisticIndex = old.posts.findIndex(
      p => 'optimistic' in p && p.optimistic && p.title === newPost.title
    );
    ...
    if (optimisticIndex !== -1) {
      ...
      return {
        ...old,
        posts: old.posts.map((p, i) => (i === optimisticIndex ? newPost : p)),
      };
    }
...
export function useCreateEntity() {
  ...
  return useMutation({
    ...
    onMutate: async newEntity => {
      ...
      queryClient.setQueryData(qk.entities.list(newEntity.parentId), old => {
        if (!old) return old;
        return {
          ...old,
          items: [
...
export async function createEntityAction(input: CreateInput) {
  ...
  if (!result[0] && result[1]) {
    ...
    await pusherServer
      .trigger(`parent-${input.parentId}-entities`, 'entity-changed', {
        type: 'INSERT',
        new: result[1],
      })
      .catch(console.error);
