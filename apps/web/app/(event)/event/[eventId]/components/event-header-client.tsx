...
export function EventHeaderClient({
  ...
  const [userMembership, setUserMembership] = useState(initialUserMembership);
  ...
  useRealtimeSync({
    ...
    onUpdate: payload => {
      // Optimistically update event details
      setEvent(prev => ({ ...prev, ...payload.new }));
    },
    refreshOnChange: true,
  });

  // Sync with realtime membership changes (for current user's RSVP)
  useRealtimeSync({
    channel: `event-${eventId}-user-membership`,
    table: 'Membership',
    filter: `id=eq.${userMembership.id}`,
    onUpdate: payload => {
      // Optimistically update user's membership (RSVP changes)
      setUserMembership(prev => ({ ...prev, ...payload.new }));
    },
    refreshOnChange: true,
  });
