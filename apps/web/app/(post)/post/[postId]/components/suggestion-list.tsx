...
function SuggestionListInner<T>(
  ...
  return (
    <div
      className={`bg-background border border-border shadow-lg p-1 min-w-[200px] max-h-[200px] overflow-y-auto ${
    ...
    >
      {items.map((item, index) => {
        ...
        return (
          ...
          >
            {prefix && isHorizontal && <div>{prefix}</div>}
            <div className={isHorizontal ? 'flex-1 min-w-0' : ''}>
              ...
              {(!prefix || isHorizontal) && (
                <div
                  className={`text-sm font-medium ${isHorizontal ? 'truncate' : ''}`}
                >
                  {primaryText}
                </div>
              )}
              {secondaryText && (
                <div
                  className={`text-xs text-muted-foreground ${isHorizontal ? 'truncate' : 'truncate'}`}
                >
                  {secondaryText}
                </div>
              )}
...
(
  SuggestionList as typeof SuggestionList & { displayName: string }
