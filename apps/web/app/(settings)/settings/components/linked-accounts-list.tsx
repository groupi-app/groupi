...
import { unlinkAccountAction } from '@/actions/account-actions';
import { toast } from 'sonner';
import { LinkedAccountData } from '@groupi/schema/data';
...
export function LinkedAccountsList({
  ...
  const handleUnlinkAccount = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error(
          error.message ||
            'Failed to unlink account. You must have at least one authentication method.'
        );
      } else {
  ...
  const pendingAccount = pendingUnlinkAccountId
    ? linkedAccounts.find(acc => acc.id === pendingUnlinkAccountId)
    : null;
  ...
  const remainingAccounts = pendingUnlinkAccountId
    ? linkedAccounts.filter(acc => acc.id !== pendingUnlinkAccountId)
    : [];
