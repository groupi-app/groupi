'use client';

import { useMutation, useQueryClient } from '@tanstack/react-query';
import { updateMemberRoleAction } from '@/actions/membership-actions';
import { qk } from '@/lib/query-keys';
import type { MembershipData } from '@groupi/schema';
import type { UpdateMemberRoleParams } from '@groupi/schema/params';
import type { MemberListPageData } from '@groupi/schema/data';
import type { MembershipMutationError } from '@/actions/membership-actions';

/**
 * Mutation hook for updating a member's role
 */
export function useUpdateMemberRole() {
  const queryClient = useQueryClient();

  return useMutation({
    // Adapter: Convert tuple to React Query format
    mutationFn: async (
      input: UpdateMemberRoleParams
    ): Promise<MembershipData> => {
      const [error, data] = await updateMemberRoleAction(input);
      if (error) throw error;
      return data;
    },
    onMutate: async (updatedRole: UpdateMemberRoleParams) => {
      // We need to get eventId from the membership - find the membership first
      // For now, we'll need to find member list queries and update optimistically
      // The server action will return eventId in the response
      const queryCache = queryClient.getQueryCache();
      const memberListQueries = queryCache.findAll({
        predicate: (query: { queryKey: readonly unknown[] }) => {
          const key = query.queryKey;
          return (
            Array.isArray(key) &&
            key[0] === 'memberships' &&
            key[1] === 'list' &&
            key.length === 3
          );
        },
      });

      // Save previous data for rollback
      const prevData = memberListQueries.map(
        (query: { queryKey: readonly unknown[]; state: { data: unknown } }) => ({
          queryKey: [...query.queryKey],
          data: query.state.data,
        })
      );

      // Optimistically update member list - we'll update all member lists
      // and filter by membershipId in each
      memberListQueries.forEach((query: { queryKey: readonly unknown[] }) => {
        const eventId = query.queryKey[2] as string | undefined;
        if (!eventId) return;

        queryClient.setQueryData<MemberListPageData>(
          qk.memberships.list(eventId),
          (old: MemberListPageData | undefined) => {
            if (!old) return old;

            return {
              ...old,
              event: {
                ...old.event,
                memberships: old.event.memberships.map(m =>
                  m.id === updatedRole.membershipId
                    ? { ...m, role: updatedRole.role }
                    : m
                ),
              },
            };
          }
        );
      });

      return { prevData };
    },
    onError: (
      _err: MembershipMutationError,
      _updatedRole: UpdateMemberRoleParams,
      ctx?: { prevData?: Array<{ queryKey: unknown[]; data: unknown }> }
    ) => {
      // Rollback on error
      if (ctx?.prevData) {
        ctx.prevData.forEach(
          ({ queryKey, data }: { queryKey: readonly unknown[]; data: unknown }) => {
            queryClient.setQueryData(queryKey, data);
          }
        );
      }
    },
    onSuccess: (data: MembershipData) => {
      const eventId = data.eventId;

      // Update with real data
      queryClient.setQueryData<MemberListPageData>(
        qk.memberships.list(eventId),
        (old: MemberListPageData | undefined) => {
          if (!old) return old;

          return {
            ...old,
            event: {
              ...old.event,
              memberships: old.event.memberships.map(m =>
                m.id === data.id ? { ...m, role: data.role } : m
              ),
            },
          };
        }
      );

      // Silently invalidate for background sync (no refetch)
      queryClient.invalidateQueries({
        queryKey: qk.memberships.list(eventId),
        refetchType: 'none',
      });
    },
  });
}
