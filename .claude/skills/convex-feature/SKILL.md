...
Create data and action hooks using the factory pattern. See the `cross-platform-hook` skill for details.

Export from `packages/shared/src/hooks/index.ts`.

## Step 7: Platform UI

Build web components using shared hooks. See the `web-component` skill for component patterns.
...
- [ ] Tests written (see `test-convex` skill)
- [ ] `pnpm check` passes
