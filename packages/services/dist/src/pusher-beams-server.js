...
import { getNEXT_PUBLIC_PUSHER_BEAMS_INSTANCE_ID, getPUSHER_BEAMS_SECRET_KEY, getNEXT_PUBLIC_BASE_URL, } from './env';
 ...
 */
export async function sendPusherBeamsNotification(userId, notification) {
    try {
        ...
        const publishResponse = await beamsClient.publishToUsers([userId], publishRequest);
        notificationLogger.info({
            userId,
            publishResponse,
            title: notification.title,
 ...
 */
export async function sendPusherBeamsNotificationToUsers(userIds, notification) {
    try {
        if (userIds.length === 0) {
        ...
        const publishResponse = await beamsClient.publishToUsers(userIds, publishRequest);
        notificationLogger.info({
            userIds,
            userCount: userIds.length,
            publishResponse,
