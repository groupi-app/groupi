...
import { createEventInviteManagementDTO, createIndividualInviteDTO, } from '@groupi/schema';
export const getEventInviteData = cache(async (eventId) => {
    try {
        ...
        // Transform to DTO
        const eventDTO = createEventInviteManagementDTO(event);
...
export const fetchInviteData = cache(async (inviteId) => {
    try {
        ...
        // Transform to DTO
        const inviteDTO = createIndividualInviteDTO(invite);
        return { success: inviteDTO };
    }
    catch {
        return { error: 'Unable to fetch invite data' };
    }
});
export async function createInvite({ name, eventId, maxUses: uses, expiresAt, }) {
    try {
        const { userId } = await auth();
        if (!userId) {
            return { error: 'User not found' };
...
export async function deleteInvite(id) {
    try {
        ...
        const membership = invite.event.memberships.find(membership => membership.personId === userId);
        if (!membership || membership.role === 'ATTENDEE') {
...
export async function deleteInvites(ids) {
    try {
        const { userId } = await auth();
        if (!userId) {
            return { error: 'User not found' };
        ...
        const membership = invites[0].event.memberships.find(membership => membership.personId === userId);
        if (!membership || membership.role === 'ATTENDEE') {
...
export async function acceptInvite({ inviteId, personId, }) {
    ...
    try {
        ...
        getPusherServer().trigger(inviteQueryDefinition.pusherChannel, inviteQueryDefinition.pusherEvent, { message: 'Data updated' });
