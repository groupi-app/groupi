...
export function useSubmitAvailability() {
  const submitAvailability = useMutation(availabilityMutations.submitAvailability);
...
export function useUpdateSingleAvailability() {
  const updateSingleAvailability = useMutation(availabilityMutations.updateSingleAvailability);
  const { toast } = useToast();
...
export function useClearAllAvailability() {
  const clearAllAvailability = useMutation(availabilityMutations.clearAllAvailability);
  const { toast } = useToast();
...
export function useAddPotentialDateTimes() {
  const addPotentialDateTimes = useMutation(availabilityMutations.addPotentialDateTimes);
  const { toast } = useToast();
...
export function useRemovePotentialDateTimes() {
  const removePotentialDateTimes = useMutation(availabilityMutations.removePotentialDateTimes);
  const { toast } = useToast();

  return useCallback(async (potentialDateTimeIds: Id<"potentialDateTimes">[]) => {
    try {
