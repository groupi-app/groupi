...
export function useCreateInvite(eventId?: Id<"events">) {
  ...
  const createInvite = useMemo(() => {
    ...
    return baseMutation.withOptimisticUpdate((localStore, args) => {
      ...
      localStore.setQuery(inviteQueries.getEventInvites, { eventId: args.eventId }, {
        ...currentData,
...
export function useUpdateInvite(eventId?: Id<"events">) {
  ...
  const updateInvite = useMemo(() => {
    ...
    return baseMutation.withOptimisticUpdate((localStore, args) => {
      ...
      const updatedInvites = currentData.invites.map((invite: { _id: Id<"invites">; name?: string; usesTotal?: number; usesRemaining?: number; expiresAt?: number }) => {
        if (invite._id === args.inviteId) {
          // Calculate new usesRemaining if usesTotal changed
          let newUsesRemaining = invite.usesRemaining;
          if (args.usesTotal !== undefined && invite.usesTotal !== undefined && invite.usesRemaining !== undefined) {
          ...
          return {
            ...
            usesTotal: args.usesTotal !== undefined ? args.usesTotal : invite.usesTotal,
            usesRemaining: newUsesRemaining,
            expiresAt: args.expiresAt !== undefined ? args.expiresAt : invite.expiresAt,
          };
        }
        return invite;
      });

      localStore.setQuery(inviteQueries.getEventInvites, { eventId }, {
        ...currentData,
...
export function useDeleteInvites(eventId?: Id<"events">) {
  ...
  const deleteInvites = useMemo(() => {
    ...
    return baseMutation.withOptimisticUpdate((localStore, args) => {
      ...
      const deletingIds = new Set(args.inviteIds.map((id: Id<"invites">) => id));
  ...
  return useCallback(async (inviteIds: Id<"invites">[]) => {
    try {
