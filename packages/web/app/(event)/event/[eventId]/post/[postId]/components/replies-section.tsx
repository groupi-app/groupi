...
export function RepliesSection({
  ...
  // Get the current user's personId from global context for filtering
  const { person } = useGlobalUser();
  const personId = person?._id as Id<'persons'> | undefined;

  // Track presence in this post using global user context
    ...
    <div className='flex flex-col mt-6'>
      ...
      {showJumpToPresent && (
        <Button
          ...
          variant='secondary'
          className={cn(
            'fixed z-float shadow-overlay text-xs w-auto',
