    ...
    <div>
      ...
      <div className='flex items-center gap-2 ml-2 mt-4'>
        <Checkbox
          ...
          checked={valid.every((invite: Doc<"invites">) => selectedInvites.includes(invite._id))}
          onCheckedChange={() => {
            if (valid.every((invite: Doc<"invites">) => selectedInvites.includes(invite._id))) {
              setSelectedInvites([]);
            } else {
              setSelectedInvites(valid.map((invite: Doc<"invites">) => invite._id));
            }
          }}
        />
        <button
      ...
      <div className='flex items-center gap-2 ml-2 mt-4'>
        <Checkbox
          ...
          checked={
            expired.length > 0 &&
            expired.every((invite: Doc<"invites">) => selectedInvites.includes(invite._id))
          }
          onCheckedChange={() => {
            if (expired.every((invite: Doc<"invites">) => selectedInvites.includes(invite._id))) {
              setSelectedInvites([]);
            } else {
              setSelectedInvites(expired.map((invite: Doc<"invites">) => invite._id));
            }
          }}
        />
        <button
