...
export async function POST(request:Request) {
    ...
    if (userId !== authorId) {
        return NextResponse.json({message: "You do not have permission to create a post for this user."}, {status: 403});
    }

    const res = await createPost({title, content, eventId, authorId});
