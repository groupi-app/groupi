...
// Send email verification email (for adding additional emails)
export const sendEmailVerificationEmail = internalMutation({
  ...
  handler: async (_ctx, { email, token }) => {
    ...
    // In development, log verification URL if debug logging is enabled
    if (isDevelopment && process.env.DEBUG_EMAIL_VERIFICATION === 'true') {
    ...
    if (!resendClient) {
      console.warn(
        'RESEND_API_KEY not configured, cannot send email verification'
      );
      return;
    }
      ...
      html: `
        ...
        <html>
          ...
          <body style="font-family: sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>Click the button below to verify this email address for your Groupi account:</p>
            <div style="margin: 30px 0;">
